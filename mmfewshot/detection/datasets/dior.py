...
DIOR_SPLIT = dict(
    ...
    ALL_CLASSES_SPLIT3=('baseballfield', 'basketballcourt', 'bridge','chimney', 'ship',   
                        'airplane', 'airport', 'Expressway-toll-station', 'harbor', 'groundtrackfield',
                        'Expressway-Service-area', 'overpass', 'stadium', 'trainstation', 'windmill', 
                        'dam', 'golffield', 'storagetank', 'tenniscourt', 'vehicle'),
    ALL_CLASSES_SPLIT4=('baseballfield', 'basketballcourt', 'bridge','chimney', 'ship',
                        'airplane', 'airport', 'Expressway-toll-station', 'harbor', 'groundtrackfield',
                        'dam', 'golffield', 'storagetank', 'tenniscourt', 'vehicle',
                        'Expressway-Service-area', 'overpass', 'stadium', 'trainstation', 'windmill'),
    ...
    BASE_CLASSES_SPLIT1=('airplane', 'airport', 'Expressway-toll-station', 'harbor', 'groundtrackfield',
                        'dam', 'golffield', 'storagetank', 'tenniscourt', 'vehicle',
                        'Expressway-Service-area', 'overpass', 'stadium', 'trainstation', 'windmill'),
    BASE_CLASSES_SPLIT2=('baseballfield', 'basketballcourt', 'bridge','chimney', 'ship',
                        'dam', 'golffield', 'storagetank', 'tenniscourt', 'vehicle',
                        'Expressway-Service-area', 'overpass', 'stadium', 'trainstation', 'windmill'),
    BASE_CLASSES_SPLIT3=('baseballfield', 'basketballcourt', 'bridge','chimney', 'ship',   
                        'airplane', 'airport', 'Expressway-toll-station', 'harbor', 'groundtrackfield',
                        'Expressway-Service-area', 'overpass', 'stadium', 'trainstation', 'windmill'),
    BASE_CLASSES_SPLIT4=('baseballfield', 'basketballcourt', 'bridge','chimney', 'ship',
                        'airplane', 'airport', 'Expressway-toll-station', 'harbor', 'groundtrackfield',
                        'dam', 'golffield', 'storagetank', 'tenniscourt', 'vehicle'),
...
class FewShotDIORDataset(BaseFewShotDataset):
    """DIOR dataset for few shot detection.

    Args:
        classes (str | Sequence[str]): Classes for model training and
            provide fixed label for each class. When classes is string,
            it will load pre-defined classes in `FewShotDIORDataset`.
    ...
    def get_classes(self, classes: Union[str, Sequence[str]]):
        ...
        ['ALL_CLASSES_SPLIT1', 'ALL_CLASSES_SPLIT2', 'ALL_CLASSES_SPLIT3', ''ALL_CLASSES_SPLIT4',
         'BASE_CLASSES_SPLIT1', 'BASE_CLASSES_SPLIT2', 'BASE_CLASSES_SPLIT3', 'BASE_CLASSES_SPLIT4',
         'NOVEL_CLASSES_SPLIT1','NOVEL_CLASSES_SPLIT2','NOVEL_CLASSES_SPLIT3', 'NOVEL_CLASSES_SPLIT4']
    ...
    def load_annotations_xml(
        ...
        for img_name in img_names:
            dataset_year = 'DIOR2017'
            # ann file in image path format

            img_id = img_name
            filename = f'DIOR2017/JPEGImages/{img_name}.jpg'
    ...
    def _get_xml_ann_info(self,
        ...
        Args:
            dataset_year (str): Year of DIOR dataset. Options are
                'DIOR2007'
    ...
    def evaluate(self,
        ...
        Args:
            ...
            class_splits: (list[str] | None): Calculate metric of classes
                split  defined in DIOR_SPLIT. For example:
...
class FewShotDIORCopyDataset(FewShotDIORDataset):
    ...
    Args:
        ann_cfg (list[dict] | dict): contain `data_infos` from other
            dataset. Example: [dict(data_infos=FewShotDIORDataset.data_infos)]
    """
    ...
    def ann_cfg_parser(self, ann_cfg: Union[List[Dict], Dict]):
        ...
        Args:
            ann_cfg (list[dict] | dict): contain `data_infos` from other
                dataset. Example:
                [dict(data_infos=FewShotDIORDataset.data_infos)]
...
class FewShotDIORDefaultDataset(FewShotDIORDataset):
    ...
    DEFAULT_ANN_CONFIG = dict(
        TFA=dior_benchmark,
        FSCE=dior_benchmark,
        Attention_RPN=dior_benchmark,
        MPSR=dior_benchmark,
        MetaRCNN=dior_benchmark,
        FSDetView=dior_benchmark)
    ...
    def ann_cfg_parser(self, ann_cfg: List[Dict]):
        ...
        return super(FewShotDIORDataset, self).ann_cfg_parser(new_ann_cfg)
