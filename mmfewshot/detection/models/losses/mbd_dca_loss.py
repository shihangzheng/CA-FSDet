...
class MBD_DCALoss(nn.Module):
    def __init__(self,
                 ...
                 a=0.5,
                 b=10):
        super(MBD_DCALoss, self).__init__()
    ...
    def forward(self, cls_score, labels, label_weights=None, decay_rate=None,**kwargs):
        ...
        if num_base > 0:
            ...
            for label in base_labels.unique():
                ...
                l_scores = scores[l_inds, label]
                if l_scores.numel() > 1:
                    l_distance = l_scores.unsqueeze(1) - l_scores.unsqueeze(0)
                    l_distance=l_distance.abs()
                    l_distance.clamp_(min=1e-7, max=self.max_diff)
                    l_distance=1-l_distance
                    l_distance.clamp_(min=1e-7, max=self.max_diff)
                    if -torch.log(l_distance).numel() > 0:
                        loss_base_intra.append(-torch.log(l_distance)*(1- l_scores[:, None]))
                        # loss_base_intra.append(-torch.log(l_distance) * (1 - l_scores[:, None]).pow(self.power_weight))
            if loss_base_intra:
                loss_base_intra = torch.cat(loss_base_intra)
                if self.reduction == 'sum':
                    loss_base_intra = loss_base_intra.sum().div(avg_factor)
                else:
                    loss_base_intra = loss_base_intra.mean()
            else:
                loss_base_intra = torch.tensor(0.0, device=scores.device)
            ...
            loss_base_margin = (loss_base_margin + loss_base_intra)*loss_base_margin_weight
        else:
        ...
        if num_novel > 0:
            ...
            for label in novel_labels.unique():
                ...
                if l_scores.numel() > 1:
                    ...
                    l_distance.clamp_(min=1e-7, max=self.max_diff)
                    if -torch.log(l_distance).numel() > 0:
                        loss_novel_intra.append(-torch.log(l_distance)*(1- l_scores[:, None]))
                        # loss_novel_intra.append(-torch.log(l_distance) * (1 - l_scores[:, None]).pow(self.power_weight))
            if loss_novel_intra:
                loss_novel_intra = torch.cat(loss_novel_intra)
                if self.reduction == 'sum':
                    loss_novel_intra = loss_novel_intra.sum().div(avg_factor)
                else:
                    loss_novel_intra = loss_novel_intra.mean()
            else:
                loss_novel_intra = torch.tensor(0.0, device=scores.device)
            ...
            loss_novel_margin = (loss_novel_margin + loss_novel_intra)*loss_novel_margin_weight
        else:
        ...
        term1 = -Y * ((2 - P) ** self.a) * torch.log(P)
        term2 = -(1 - Y) * ((1 + P) ** self.b) * torch.log(1 - P)
        encls_loss = (term1 + term2)
        if self.reduction == 'sum':
            encls_loss = encls_loss.sum().div(avg_factor)
        else:
            encls_loss = encls_loss.mean()
        encls_loss*=0.0001
        losses['loss_encls'] = encls_loss
        ...
        if self.reduction == 'sum':
            ...
            if 'loss_encls' in losses:
                losses['loss_encls'].clamp_(max=self.max_loss)
