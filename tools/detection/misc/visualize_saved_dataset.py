...
class Visualizer:
    ...
    def visualize(self,
        ...
        instances = {target_class: []}  # 只保留目标类别
        for data_info in self.data_infos:
            ...
            for i in range(len(labels)):
                if labels[i] == target_class:  # 只处理目标类别的实例
                    bbox = list(map(int, bboxes[i]))
                    cv2.rectangle(img, (bbox[0], bbox[1]), (bbox[2], bbox[3]),
                                  self.color_map[labels[i]], 2)
                    croped_instance = self.crop_instance(img, bboxes[i],
                                                         instance_size)
                    instances[target_class].append(croped_instance)

            image_name = f'{save_name}_class_{target_class}.jpg'
            img_out = self.concat_images(instances, num_cols, instance_size)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help='The index of the class to visualize')
...
if __name__ == '__main__':
    ...
    visualizer.visualize(args.save_name, num_cols=args.col, target_class=args.class_index)
